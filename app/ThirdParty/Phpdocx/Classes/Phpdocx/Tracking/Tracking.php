...
class Tracking
{
    ...
    public function acceptTracking($domDocument, $domXpath, $query)
    {
        ...
        if ($contentNodes->length > 0) {
            foreach ($contentNodes as $contentNode) {
                ...
                // w:sectPrChange remove node and keep the contents
                $querySectPrChange = './/w:sectPrChange';
                $sectPrChangeNodes = $contentNodeXPath->query($querySectPrChange, $contentNode);
                if ($sectPrChangeNodes->length > 0) {
                    foreach ($sectPrChangeNodes as $sectPrChangeNode) {
                        $sectPrChangeNode->parentNode->removeChild($sectPrChangeNode);
                    }
                }
    ...
    public function addPerson($people, $person)
    {
        ...
        if ($personNodes->length > 0) {
            // the person exists
            PhpdocxLogger::logger('This person is already in the people file', 'warning');
        } else {
    ...
    public function addTrackingInsFirstR($xml)
    {
        ...
        if ($posRpr !== false) {
            $xml = substr_replace($xml, '<w:ins w:author="'.\Phpdocx\Create\CreateDocx::$trackingOptions['author'].'" w:date="'.\Phpdocx\Create\CreateDocx::$trackingOptions['date'].'" w:id="'.\Phpdocx\Create\CreateDocx::$trackingOptions['id'].'">' . $tag, $posRpr, strlen($tag));

            \Phpdocx\Create\CreateDocx::$trackingOptions['id'] = \Phpdocx\Create\CreateDocx::$trackingOptions['id'] + 1;
    ...
    public function addTrackingInsR($xml)
    {
        ...
        foreach ($rNodes as $rNode) {
            ...
            $insNode->setAttribute('w:author', \Phpdocx\Create\CreateDocx::$trackingOptions['author']);
            $insNode->setAttribute('w:date', \Phpdocx\Create\CreateDocx::$trackingOptions['date']);
            $insNode->setAttribute('w:id', \Phpdocx\Create\CreateDocx::$trackingOptions['id']);
    ...
    public function rejectTracking($domDocument, $domXpath, $query)
    {
        ...
        if ($contentNodes->length > 0) {
            foreach ($contentNodes as $contentNode) {
                ...
                // w:sectPrChange move nodes and remove the previous w:sectPrChange
                $querySectPrChange = './/w:sectPrChange';
                $sectPrChangeNodes = $contentNodeXPath->query($querySectPrChange, $contentNode);
                if ($sectPrChangeNodes->length > 0) {
                    $nodesToBeRemoved = array();
                    foreach ($sectPrChangeNodes as $sectPrChangeNode) {
                        $sectPrChangeNodeClone = $sectPrChangeNode->firstChild->cloneNode(true);
                        $sectPrChangeNode->parentNode->parentNode->insertBefore($sectPrChangeNodeClone, $sectPrChangeNode->parentNode);
                        $nodesToBeRemoved[] = $sectPrChangeNode;
                    }
