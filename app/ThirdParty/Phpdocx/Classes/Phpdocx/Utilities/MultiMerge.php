...
class MultiMerge
{
     ...
     * @access private
     */
    private $_takenNumberingsIdsDocument;
     ...
     * 'forceLatestStyles' (bool) if true (default is false) uses the latest document as the base styles
     * 'lineBreaks' (int) inserts the number of line breaks indicated between the contents of the merging files
     * 'mergeType' (int) (0, 1) preserves or not the sections of the merged document respectively
     * 'numbering' (string) (continue, restart) allows to restart the page numbering in the merged document
     * 'renameStyles' (bool) false as default, styles with the same name aren't added to the merge DOCX. If true, rename styles of the DOCX to be merged with unique names to keep them
     * 'preserveStyleDefaults' (bool) if true (default) makes sure that the defaults of the first document are not overriden
     * @return DOCXStructure
     */
    public function mergeDocx($firstDocument, $documentArray, $finalDocument, $options)
    {
        // initialize the required options
        if (!isset($options['preserveStyleDefaults'])) {
            $this->_preserveDefaults = true;
        } else {
            $this->_preserveDefaults = $options['preserveStyleDefaults'];
        }
        if (!isset($options['renameStyles'])) {
            $this->_renameStyles = false;
        } else {
            $this->_renameStyles = $options['renameStyles'];
        }
        $this->_background = '';
        $this->_wordMLChunk = '';
        ...
        $this->_takenNumberingsIdsDocument = array();
        ...
        foreach ($documentArray as $key => $secondDocument) {
            ...
            // reset taken numberings Ids of the current document to avoid getting ids from other documents
            $this->_takenNumberingsIdsDocument = array();
            ...
            // rename styles
            $stylesRenamed = null; 
            if ($this->_renameStyles) {
                $stylesRenamed = $this->stylesRenameStructuralData($this->_secondDocxStructuralData);
            }
            ...
            for ($j = 1; $j <= count($this->_secondDocxStructuralData['charts']); $j++) {
                foreach ($this->_secondDocxStructuralData['charts'][$j] as $key => $value) {
                    ...
                    $xlsxTargetMode = $xlsxNode->getAttribute('TargetMode');
                    // get the original name of the xlsx file
                    ...
                    // change the target attribute if the chart is not external
                    if ($xlsxTargetMode != 'External') {
                    ...
                    // and the corresponding excel in the embeddings folder it the element is not External
                    if ($xlsxTargetMode != 'External') {
            ...
            if ($this->checkData($this->_secondDocxStructuralData['numberings']) > 0) {
                ...
                if ($this->_secondDocxNumberingXML) {
                    // load the numberings into the DOM
                    if (PHP_VERSION_ID < 80000) {
                        $optionEntityLoader = libxml_disable_entity_loader(true);
                    }
            ...
            if ($this->_renameStyles) {
                $this->mergeStylesRename($this->_firstStylesDOM, $this->_secondStylesDOM, $stylesRenamed, $this->_firstDocxStructuralData);
            } else {
    ...
    public function getDocxStructuralData($docDOM, $relsDOM, $contentTypesDOM, $relabel = false)
    {
        ...
        // Main content
        // these arrays keep repeated images, shapes and links in images
        $imageShapeIds = array();
        $imageShapeFillIds = array();
        $imageShapeBlipFillIds = array();
        $imageSvgBlip = array();
        $originalIds = array();
        $altchunkIds = array();
        $imageLinkIds = array();
        ...
        for ($k = 1; $k <= $numSections; $k++) {
            ...
            $docXPath->registerNamespace('v', 'urn:schemas-microsoft-com:vml');
            $docXPath->registerNamespace('o', 'urn:schemas-microsoft-com:office:office');
            $docXPath->registerNamespace('asvg', 'http://schemas.microsoft.com/office/drawing/2016/SVG/main');
            // look for images
            ...
            foreach ($imageNodes as $node) {
                ...
                if ($attr) {
                    ...
                    if ($relabel) {
                        ...
                        // update the document.xml.rels file
                        $query = '//rels:Relationship[@Id="' . $attr . '"]';
                        $affectedNodes = $relsXPath->query($query);
                        $nodeToBeChanged = $affectedNodes->item(0);
                        if (is_object($nodeToBeChanged)) {
            ...
            // images fill shapes, v:fill tag
            $queryImageFillShape = '//v:fill';
            $imageFillShapeNodes = $docXPath->query($queryImageFillShape);
            foreach ($imageFillShapeNodes as $node) {
                $attr = $node->getAttribute('r:id');
                if ($attr) {
                    $extArray = explode('.', $relsArray[$attr]);
                    $extension = array_pop($extArray);
                    if (key_exists($attr, $imageShapeFillIds)) {
                        $myId = $imageShapeFillIds[$attr];
                        $newId = 'rId' . $myId;
                    } else {
                        $myId = uniqid(mt_rand(999, 9999));
                        $newId = 'rId' . $myId;
                        $imageShapeFillIds[$attr] = $myId;
                    }
                    $newPath = 'media/image' . $myId . '.' . $extension;
                    $images[$k][$attr] = array(
                        'path' => $relsArray[$attr],
            ...
            // images fill shapes, a:blipFill/a:blip tag
            $queryImage = '//a:blipFill/a:blip';
            $imageNodes = $docXPath->query($queryImage);
            foreach ($imageNodes as $node) {
                $link = '';
                $attr = $node->getAttribute('r:embed');
                if ($attr) {
                    ...
                    $extArray = explode('.', $relsArray[$attr]);
                    $extension = array_pop($extArray);
                    if (key_exists($attr, $imageShapeBlipFillIds)) {
                        $myId = $imageShapeBlipFillIds[$attr];
                        $newId = 'rId' . $myId;
                    } else {
                        $myId = uniqid(mt_rand(999, 9999));
                        $newId = 'rId' . $myId;
                        $imageShapeBlipFillIds[$attr] = $myId;
                    }
                    if (key_exists($link, $imageShapeBlipFillIds)) {
                        $myIdLink = $imageShapeBlipFillIds[$link];
                        $newIdLink = 'rId' . $myIdLink;
                    } else if (!empty($link)){
                        $myIdLink = uniqid(mt_rand(999, 9999));
                        $newIdLink = 'rId' . $myIdLink;
                        $imageShapeBlipFillIds[$link] = $myIdLink;
                    }
                    $newPath = 'media/image' . $myId . '.' . $extension;
                    $images[$k][$attr] = array(
                        'path' => $relsArray[$attr],
            ...
            // SVG images, a:blip/asvg:svgBlip tag
            $queryImage = '//a:blip//asvg:svgBlip';
            $imageNodes = $docXPath->query($queryImage);
            foreach ($imageNodes as $node) {
                $link = '';
                $attr = $node->getAttribute('r:embed');
                if ($attr) {
                    ...
                    $extArray = explode('.', $relsArray[$attr]);
                    $extension = array_pop($extArray);
                    if (key_exists($attr, $imageSvgBlip)) {
                        $myId = $imageSvgBlip[$attr];
                        $newId = 'rId' . $myId;
                    } else {
                        $myId = uniqid(mt_rand(999, 9999));
                        $newId = 'rId' . $myId;
                        $imageSvgBlip[$attr] = $myId;
                    }
                    if (key_exists($link, $imageSvgBlip)) {
                        $myIdLink = $imageSvgBlip[$link];
                        $newIdLink = 'rId' . $myIdLink;
                    } else if (!empty($link)){
                        $myIdLink = uniqid(mt_rand(999, 9999));
                        $newIdLink = 'rId' . $myIdLink;
                        $imageSvgBlip[$link] = $myIdLink;
                    }
                    $newPath = 'media/image' . $myId . '.' . $extension;
                    $images[$k][$attr] = array(
                        'path' => $relsArray[$attr],
            ...
            // links in images
            $queryHlinkclick = '//a:hlinkClick';
            $queryHlinkclickNodes = $docXPath->query($queryHlinkclick);
            foreach ($queryHlinkclickNodes as $node) {
                $attr = $node->getAttribute('r:id');
                if (key_exists($attr, $imageLinkIds)) {
                    $myId = $imageLinkIds[$attr];
                    $newId = 'rId' . $myId;
                } else {
                    $myId = uniqid(mt_rand(999, 9999));
                    $newId = 'rId' . $myId;
                    $imageLinkIds[$attr] = $myId;
                }
                $links[$k][$attr] = array(
                    'path' => $relsArray[$attr],
                    'newId' => $newId
            ...
            foreach ($afChunkNodes as $node) {
                $attr = $node->getAttribute('r:id');
                $myId = uniqid(mt_rand(999, 9999));
                if (key_exists($attr, $altchunkIds)) {
                    $myId = $altchunkIds[$attr];
                    $newId = 'altChunk' . $myId;
                } else {
                    $myId = uniqid(mt_rand(999, 9999));
                    $newId = 'altChunk' . $myId;
                    $altchunkIds[$attr] = $myId;
        ...
        for ($j = 1; $j <= count($numberings); $j++) {
            ...
            foreach ($numberings[$j] as $key => $value) {
                if (!in_array($key, $refNumberings)) {
                    ...
                    // get the associated numbering style but first check that $numNodes is not empty
                    if ($numNodes->length > 0) {
                        ...
                        if ($absNumNodes->length > 0) {
                            ...
                            // if number exists reuse it
                            if (isset($newNumberings[$absNum])) {
                                $newAbstractNumId = $newNumberings[$absNum];
                            } else {
                                $newNumberings[$absNum] = $value;
                            ...
                            if ($numNodes->item(0)->getElementsByTagName('lvlOverride')->length > 0) {
                                ...
                                if (isset($newNumberings[$absNum])) {
                                  $absNum = $newNumberings[$absNum];
                                }
                                // lvlOverride enabled, use the source abstractNumId
    ...
    private function mergeNumberingsStyles($stylesDOM, $firstNumberingDOM, $secondNumberingDOM)
    {
        ...
        foreach ($numberingNodes as $node) {
            ...
            if (!isset($numberings[1][$attr])) {
                $numberings[1][$attr] = $this->uniqueDecimal($this->_takenNumberingsIds, 999, 9999);
                if ($this->_renameStyles) {
                    foreach ($this->_takenNumberingsIdsDocument as $takenNumberingIdsDocument) {
                        if ($takenNumberingIdsDocument['id'] == $attr) {
                            $numberings[1][$attr] = $takenNumberingIdsDocument['value'];
                        }
                    }
                }
                $this->_takenNumberingsIds[] = $numberings[1][$attr];
    ...
    /**
     * Merges the required styles into a single file renaming the second styles
     * @access private
     * @param DOMDocument $myOriginalStyles
     * @param DOMDocument $myMergedStyles
     * @param array $stylesRenamed
     * @param array $structuralData
     * @return string
     */
    private function mergeStylesRename($myOriginalStyles, $myMergedStyles, $stylesRenamed, $structuralData)
    {
        // prepare $myMergedStyles and $myOriginalStyles for XPath searches of the required nodes
        $mergedXPath = new \DOMXPath($myMergedStyles);
        $mergedXPath->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');
        ...
        // keep replaced styles
        $replacedStyles = array();
        // fake array to keep used keys
        $array = array();
        if (is_array($stylesRenamed) && count($stylesRenamed) > 0) {

            // if the style needs to be renamed, rename and add it
            foreach ($stylesRenamed as $styleRenamedOld => $styleRenamedNew) {
                // w:styleId
                $query = '//w:style[@w:styleId="' . $styleRenamedOld . '"]';
                $mergedStyleNodes = $mergedXPath->query($query);
                foreach ($mergedStyleNodes as $node) {
                    $replacedStyles[$styleRenamedOld] = $styleRenamedNew;
                    $node->setAttribute('w:styleId', $styleRenamedNew);

                    // w:name
                    $nodeName = $node->getElementsByTagName('name');
                    if ($nodeName->length > 0) {
                        $nameNameValue = $nodeName->item(0)->getAttribute('w:val');
                        if (!isset($replacedStyles[$nameNameValue])) {
                            $nameNameNewValue = $nameNameValue . $this->uniqueDecimal($array, 999, 9999);
                            $replacedStyles[$nameNameValue] = $nameNameNewValue;
                        } else {
                            $nameNameNewValue = $replacedStyles[$nameNameValue];
                        }
                        // w:styleId
                        $queryStyleName = '//w:style[@w:styleId="' . $nameNameValue . '"]';
                        $styleNodesName = $mergedXPath->query($queryStyleName);
                        foreach ($styleNodesName as $styleNodeName) {
                            $styleNodeName->setAttribute('w:styleId', $nameNameNewValue);
                        }
                        // w:name tags
                        $queryStyleName = '//w:name[@w:val="' . $nameNameValue . '"]';
                        $styleNodesName = $mergedXPath->query($queryStyleName);
                        foreach ($styleNodesName as $styleNodeName) {
                            $styleNodeName->setAttribute('w:val', $nameNameNewValue);
                        }
                    }

                    // w:basedOn
                    $nodeBasedOn = $node->getElementsByTagName('basedOn');
                        if ($nodeBasedOn->length > 0) {
                          $basedOnNameValue = $nodeBasedOn->item(0)->getAttribute('w:val');
                          if (!isset($replacedStyles[$basedOnNameValue])) {
                                $basedOnNameNewValue = $basedOnNameValue . $this->uniqueDecimal($array, 999, 9999);
                                $replacedStyles[$basedOnNameValue] = $basedOnNameNewValue;
                          } else {
                                $basedOnNameNewValue = $replacedStyles[$basedOnNameValue];
                          }
                          // w:styleId
                          $queryStyleBasedOn = '//w:style[@w:styleId="' . $basedOnNameValue . '"]';
                          $styleNodesBasedOn = $mergedXPath->query($queryStyleBasedOn);
                          foreach ($styleNodesBasedOn as $styleNodeBasedOn) {
                                $styleNodeBasedOn->setAttribute('w:styleId', $basedOnNameNewValue);
                          }
                          // w:basedOn tags
                          $queryStyleBasedOn = '//w:basedOn[@w:val="' . $basedOnNameValue . '"]';
                          $styleNodesBasedOn = $mergedXPath->query($queryStyleBasedOn);
                          foreach ($styleNodesBasedOn as $styleNodeBasedOn) {
                                $styleNodeBasedOn->setAttribute('w:val', $basedOnNameNewValue);
                          }
                    }

                    // w:next
                    /*$nodeNext = $node->getElementsByTagName('next');
                    if ($nodeNext->length > 0) {
                      $nextNameValue = $nodeNext->item(0)->getAttribute('w:val');
                      if (!isset($replacedStyles[$nextNameValue])) {
                        $nextNameNewValue = $nextNameValue . $this->uniqueDecimal($array, 999, 9999);
                        $replacedStyles[$nextNameValue] = $nextNameNewValue;
                      } else {
                        $nextNameNewValue = $replacedStyles[$nextNameValue];
                      }
                      // w:styleId
                      $queryStyleNext = '//w:style[@w:styleId="' . $nextNameValue . '"]';
                      $styleNodesNext = $mergedXPath->query($queryStyleNext);
                      foreach ($styleNodesNext as $styleNodeNext) {
                        $styleNodeNext->setAttribute('w:styleId', $nextNameNewValue);
                      }
                      // w:next tags
                      $queryStyleNext = '//w:next[@w:val="' . $nextNameValue . '"]';
                      $styleNodesNext = $mergedXPath->query($queryStyleNext);
                      foreach ($styleNodesNext as $styleNodeNext) {
                        $styleNodeNext->setAttribute('w:val', $nextNameNewValue);
                      }
                    }*/

                    // w:link
                    /*$nodeLink = $node->getElementsByTagName('link');
                    if ($nodeLink->length > 0) {
                      $linkNameValue = $nodeLink->item(0)->getAttribute('w:val');
                      if (!isset($replacedStyles[$linkNameValue])) {
                        $linkNameNewValue = $linkNameValue . $this->uniqueDecimal($array, 999, 9999);
                        $replacedStyles[$linkNameValue] = $linkNameNewValue;
                      } else {
                        $linkNameNewValue = $replacedStyles[$linkNameValue];
                      }
                      // w:styleId
                      $queryStyleLink = '//w:style[@w:styleId="' . $linkNameValue . '"]';
                      $styleNodesLink = $mergedXPath->query($queryStyleLink);
                      foreach ($styleNodesLink as $styleNodeLink) {
                        $styleNodeLink->setAttribute('w:styleId', $linkNameNewValue);
                      }
                      // w:link tags
                      $queryStyleLink = '//w:link[@w:val="' . $linkNameValue . '"]';
                      $styleNodesLink = $mergedXPath->query($queryStyleLink);
                      foreach ($styleNodesLink as $styleNodeLink) {
                        $styleNodeLink->setAttribute('w:val', $linkNameNewValue);
                      }
                    }*/

                    // w:numId for pStyle, if a paragraph contains a pStyle and a w:numPr, the style tag with the same styleId in the styles file must have the same numId
                    // not needed after the changes applied to the mergeNumberingsStyles method to handle list IDs
                    /*foreach ($structuralData['section'] as $contentSectionKey => $contentSectionValue) {
                        // w:pStyle
                        $contentSectionXPath = new \DOMXPath($contentSectionValue);
                        $contentSectionXPath->registerNamespace('w', 'http://schemas.openxmlformats.org/wordprocessingml/2006/main');
                        $query = '//w:pStyle[@w:val="' . $styleRenamedNew . '"]';
                        $nodesPStyle = $contentSectionXPath->query($query);
                        if ($nodesPStyle && $nodesPStyle->length > 0) {
                            // only if the parent element has a w:numPr/w:numId
                            foreach ($nodesPStyle as $nodePStyle) {
                                $numPrNodeNumPr = $nodePStyle->parentNode->getElementsByTagName('numPr');
                                if ($numPrNodeNumPr && $numPrNodeNumPr->length > 0) {
                                    $numPrNodeNumId = $numPrNodeNumPr->item(0)->getElementsByTagName('numId');
                                    if ($numPrNodeNumId && $numPrNodeNumId->length > 0) {
                                        // $pStyleNumId is the numId to be applied to the style tag in the styles file
                                        $pStyleNumId = $numPrNodeNumId->item(0)->getAttribute('w:val');
                                        // w:styleId
                                        $query = '//w:style[@w:styleId="' . $styleRenamedNew . '"]//w:numId';
                                        $styleNodesNew = $mergedXPath->query($query);
                                        if ($styleNodesNew && $styleNodesNew->length > 0) {
                                            foreach ($styleNodesNew as $styleNodeNew) {
                                                $styleNodeNew->setAttribute('w:val', $pStyleNumId);
                                            }
                                        }
                                    }
                                }
                        ...
                        // overwrite the section based on the key
                        $structuralData['section'][$contentSectionKey] = $contentSectionValue;
                    }*/
                }
            }
        }

        // rename w:name in w:default="1" styles from the document to be merged
        $query = '//w:style[@w:default="1"]';
        $mergedStyleNodesDefault = $mergedXPath->query($query);
        foreach ($mergedStyleNodesDefault as $mergedStyleNodeDefault) {
            $nodeName = $mergedStyleNodeDefault->getElementsByTagName('name');
            if ($nodeName->length > 0) {
                $nameNameValue = $nodeName->item(0)->getAttribute('w:val');
                if (!isset($replacedStyles[$nameNameValue])) {
                    $nameNameNewValue = $nameNameValue . $this->uniqueDecimal($array, 999, 9999);
        ...
        // extract the style nodes from the file to be merged
        $query = '//w:style';
        $mergedStyleNodes = $mergedXPath->query($query);
        foreach ($mergedStyleNodes as $node) {
            $styleId = $node->getAttribute('w:styleId');
            // check if that style already exists in the original file
            $query = '//w:style[@w:styleId="' . $styleId . '"]';
            $foundNodes = $originalXPath->query($query);
            if ($foundNodes->length == 0) {
                $newStyleNode = $myOriginalStyles->importNode($node, true);
    ...
    private function mergeRels($myOriginalRels, $myMergedRels)
    {
        ...
        foreach ($mergedRelsNodes as $node) {
            ...
            if (strstr($target, 'customXml') === false &&
                ...
                $foundNodes = $originalXPath->query($query);
                if ($foundNodes->length == 0 || $target == '#toc') {
    ...
    /**
     * Rename styles in the structural data
     * @return array New styles
     */
    private function stylesRenameStructuralData($structuralData)
    {
        $newStyles = array();
        // fake array to keep used keys
        $array = array();
        foreach ($structuralData['section'] as $contentSectionKey => $contentSectionValue) {
            ...
            if ($nodesPStyle) {
                foreach ($nodesPStyle as $nodePStyle) {
                    if ($nodePStyle->hasAttribute('w:val')) {
                        // do not generate new style names for previous styles with the same name
                        if (!isset($newStyles[$nodePStyle->getAttribute('w:val')])) {
                          $decimalNumber = $this->uniqueDecimal($array, 999, 9999);
                          $newStyle = $nodePStyle->getAttribute('w:val') . $decimalNumber;
                          $newStyles[$nodePStyle->getAttribute('w:val')] = $newStyle;
                          $nodePStyle->setAttribute('w:val', $newStyle);
                        } else {
                          $nodePStyle->setAttribute('w:val', $newStyles[$nodePStyle->getAttribute('w:val')]);
                        }
                    }
                }
            }
            ...
            // overwrite the section based on the key
            $structuralData['section'][$contentSectionKey] = $contentSectionValue->saveXML();
        }
