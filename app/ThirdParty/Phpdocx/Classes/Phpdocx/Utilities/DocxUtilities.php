...
class DocxUtilities
{
    ...
    public function addWordMLContent($document, $docx, $fragment)
    {
        if ($docx instanceof CreateDocx && $fragment instanceof WordMLFragment) {
            ...
            PhpdocxLogger::logger('The PHPDocX document object or the WordMLFragment you have passed as an argument do not exist.', 'fatal');
        }
    }
    ...
    public function watermarkDocx($source, $target, $type = 'text', $options = array())
    {
        ...
        if ($type == 'image') {
            ...
            if (!in_array($extension, $ext)) {
                PhpdocxLogger::logger('Invalid image extension.', 'fatal');
            }
        ...
        foreach ($sectNodes as $node) {
            ...
            if ($headerNodes->length > 0) {
                foreach ($headerNodes as $header) {
                    ...
                    //We call the method that does the insertions in the header files
                    if ($type == 'text') {
    ...
    private function watermarkInsertImage($zip, $options, $headerFile, $headerContents, $imageId)
    {
        ...
        if (isset($options['image'])) {
            ...
            PhpdocxLogger::logger('There is no image path.', 'fatal');
        }
        //height and width
        ...
        foreach ($sectNodes as $node) {
            ...
            if ($headerNodes->length > 0) {
                foreach ($headerNodes as $header) {
                    $id = $header->getAttribute('r:id');
                    $query = '//rels:Relationship[@Id="' . $id . '"]';
                    $targetRels = $this->_watermarkRelsXPath->query($query)->item(0)->getAttribute('Target');
                    // open the corresponding header.xml
                    $headerContents = $this->_watermarkDocx->getContent('word/' . $targetRels);
                    $headerDOM = new \DOMDocument();
                    if (PHP_VERSION_ID < 80000) {
                        $optionEntityLoader = libxml_disable_entity_loader(true);
                    }
                    ...
                    $this->_watermarkDocx->addContent('word/' . $targetRels, $headerDOM->saveXML());
                }
            }

            $footerNodes = $node->getElementsByTagName('footerReference');
            if ($footerNodes->length > 0) {
                foreach ($footerNodes as $footer) {
                    $id = $footer->getAttribute('r:id');
                    $query = '//rels:Relationship[@Id="' . $id . '"]';
                    $targetRels = $this->_watermarkRelsXPath->query($query)->item(0)->getAttribute('Target');
                    // open the corresponding footer.xml 
                    $footerContents = $this->_watermarkDocx->getContent('word/' . $targetRels);
                    $footerDOM = new \DOMDocument();
                    if (PHP_VERSION_ID < 80000) {
                        $optionEntityLoader = libxml_disable_entity_loader(true);
                    }
                    $footerDOM->loadXML($footerContents);
                    if (PHP_VERSION_ID < 80000) {
                        libxml_disable_entity_loader($optionEntityLoader);
                    }
                    ...
                    $footerXpath->registerNamespace('ve', 'http://schemas.openxmlformats.org/markup-compatibility/2006');
                    $footerXpath->registerNamespace('o', 'urn:schemas-microsoft-com:office:office');
                    $footerXpath->registerNamespace('r', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships');
                    $footerXpath->registerNamespace('m', 'http://schemas.openxmlformats.org/officeDocument/2006/math');
                    $footerXpath->registerNamespace('wne', 'http://schemas.microsoft.com/office/word/2006/wordml');
                    $footerXpath->registerNamespace('v', 'urn:schemas-microsoft-com:vml');
                    $footerXpath->registerNamespace('wp', 'http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing');
                    $footerXpath->registerNamespace('w10', 'urn:schemas-microsoft-com:office:word');
                    $query = '//v:shape[@type="#_x0000_t136"] | //v:shape[@type="#_x0000_t75"]';
                    $watermarkNodes = $footerXpath->query($query);
                    foreach ($watermarkNodes as $waterNode) {
                        $waterNode->parentNode->parentNode->parentNode->parentNode->removeChild($waterNode->parentNode->parentNode->parentNode);
                    }
                    $this->_watermarkDocx->addContent('word/' . $targetRels, $footerDOM->saveXML());
                }
            }
        }

        $this->_watermarkDocx->saveDocx($target);
    }
    ...
    public function replaceChartData($source, $target, $chartData)
    {
        ...
        $tempRnd = uniqid(mt_rand(999, 9999));
        ...
        foreach ($chartData as $idChart => $data) {
            ...
            $chartStructure = $excel->createXlsx('chartData' . $tempRnd . str_replace('rId', '', $idCharts[$idChart]) . '.xlsx', $charData);
            $chartStructure->saveDocx($this->getTempDir() . '/' . 'chartData' . $tempRnd . str_replace('rId', '', $idCharts[$idChart]) . '.xlsx');
            rename($this->getTempDir() . '/chartData' . $tempRnd . str_replace('rId', '', $idCharts[$idChart]) . '.xlsx.docx', $this->getTempDir() . '/chartData' . $tempRnd . str_replace('rId', '', $idCharts[$idChart]) . '.xlsx');
            ...
            $zip->addFile($this->getTempDir() . '/chartData' . $tempRnd . str_replace('rId', '', $idCharts[$idChart]) . '.xlsx', str_replace('../', 'word/', $chartTarget));
        }
        
        $zip->close();
        foreach ($chartData as $idChart => $data) {
            unlink($this->getTempDir() . '/chartData' . $tempRnd . str_replace('rId', '', $idCharts[$idChart]) . '.xlsx');
        }
    }
