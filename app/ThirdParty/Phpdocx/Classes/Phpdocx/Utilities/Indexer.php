...
class Indexer
{
    ...
    protected function extractSignature($xml)
    {
        ...
        $this->documentStructure['signatures'][] = array(
    ...
    /**
     * Extract sources from a XML string
     *
     * @param string $xml XML string
     */
    protected function extractSources($xml)
    {
        // load XML content
        $contentDOM = new \DOMDocument();
        if (PHP_VERSION_ID < 80000) {
        ...
        // do a global xpath query getting only source tags
        $contentXpath = new \DOMXPath($contentDOM);
        $contentXpath->registerNamespace('b', 'http://schemas.openxmlformats.org/officeDocument/2006/bibliography');
        $sourceEntries = $contentXpath->query('//b:Source');
        ...
        foreach ($sourceEntries as $sourceEntry) {
            ...
            $type = '';
            $typeNodes = $sourceEntry->getElementsByTagNameNS('http://schemas.openxmlformats.org/officeDocument/2006/bibliography', 'SourceType');
            if ($typeNodes->length > 0) {
                $type = $typeNodes->item(0)->textContent;
            }
                ...
                foreach ($queryPersonNodes as $queryPersonNode) {
                    ...
                    if ($childrenPersonNodes->length > 0) {
                        ...
                        foreach ($childrenPersonNodes as $childrenPersonNode) {
                            $tagName = str_replace('b:', '', $childrenPersonNode->tagName);
                            $authorData[$tagName] = $childrenPersonNode->textContent;
                        }
            ...
            $info = array();
            $excludeTags = array('Tag', 'SourceType', 'Author');
            $childrenSourceNodes = $sourceEntry->childNodes;
            if ($childrenSourceNodes->length > 0) {
                foreach ($childrenSourceNodes as $childrenSourceNode) {
                    $tagName = str_replace('b:', '', $childrenSourceNode->tagName);
                    // avoid tags added in previous steps
                    if (in_array($tagName, $excludeTags)) {
                        continue;
                    }
                    $info[$tagName] = $childrenSourceNode->textContent;
                }
            }

            $this->documentStructure['sources'][] = array('tag' => $tag, 'type' => $type, 'authors' => $authors, 'info' => $info);
        }
    }
    ...
    private function parse($source)
    {
        ...
        foreach ($contentTypesXml->Override as $override) {
            foreach ($override->attributes() as $attribute => $value) {
                ...
                } else if ($value == 'application/vnd.openxmlformats-officedocument.customXmlProperties+xml') {
                    // source contents. Get item1.xml that is the file that contains the sources

                    $contentSources = $this->documentZip->getContent('customXml/item1.xml');
                    if ($contentSources) {
                        $this->extractSources($contentSources);
                    }
                }
            }
        }
