...
class BulkProcessing
{
    ...
    public function __construct($template, $templateSymbolStart = '$', $templateSymbolEnd = '$')
    {
        ...
        // keep init CreateDocx::$returnDocxStructure value to restore the previous value instead of the default one
        $docxStructureValue = CreateDocx::$returnDocxStructure;
        ...
        CreateDocx::$returnDocxStructure = $docxStructureValue;
        ...
        foreach ($this->cachedContents as $cacheContent) {
            ...
            if (count($cacheContent['wordfragments']) > 0) {
                ...
                foreach ($cacheContent['wordfragments'] as $wordfragmentKey => $wordfragmentValue) {
                    if (!($wordfragmentValue['wordfragment'] instanceof WordFragment) && is_array($wordfragmentValue['wordfragment'])) {
                        $elementOptions = $wordfragmentValue['wordfragment']['options'];
                        switch ($wordfragmentValue['wordfragment']['type']) {
                            case 'altchunk':
                                $wordfragmentValue['wordfragment'] = new WordFragment($docxTemplate);
                                $wordfragmentValue['wordfragment']->addExternalFile($elementOptions);
                                break;
                            case 'chart':
                                $wordfragmentValue['wordfragment'] = new WordFragment($docxTemplate);
                                $wordfragmentValue['wordfragment']->addChart($elementOptions);
                                break;
                            case 'image':
                                $wordfragmentValue['wordfragment'] = new WordFragment($docxTemplate);
                                $wordfragmentValue['wordfragment']->addImage($elementOptions);
                                break;
                            default:
                                break;
                        }
                    }
                    if ($wordfragmentValue['wordfragment'] instanceof WordFragment) {
                        if (is_array($wordfragmentValue['options']['target'])) {
                            foreach ($wordfragmentValue['options']['target'] as $optionTarget) {
                                if ($optionTarget == 'header') {
                                    $optionTarget = 'defaultHeader';
                                } else if ($optionTarget == 'footer') {
                                    $optionTarget = 'defaultFooter';
                                }
                                $docxTemplate->replaceVariableByWordFragment(array($wordfragmentKey => $wordfragmentValue['wordfragment']), array('type' => $wordfragmentValue['options']['type'], 'target' => $optionTarget));
                            }
                        } else {
                            $optionTarget = $wordfragmentValue['options']['target'];
                            if ($optionTarget == 'header') {
                                $optionTarget = 'defaultHeader';
                            } else if ($optionTarget == 'footer') {
                                $optionTarget = 'defaultFooter';
     ...
     * @access public
     * @param array $variables
     *        variable names: WordFragment or array values . Allows setting an array of array to generate more than one document output
     *              Supported WordFragment types with extenal dependencies: altchunk, chart, image
     * @param array $options
     *        'firstMatch' (boolean) if true it only replaces the first variable match. Default is set to false
    ...
    protected function replaceListContents($variables, $dom, $options)
    {
        ...
        foreach ($variables as $valueList) {
            ...
            foreach ($valueList as &$varsRow) {
                $this->replaceListWordFragments($wordFragmentsValues, $varsRow);
            }
            ...
            foreach ($foundNodes as $node) {
                ...
                if ($numIlvlNode->length > 0) {
                    $this->replaceListValues($search, $domNode, $valuesList[0], (int)$numIlvlNode->item(0)->getAttribute('w:val'));
                } else {
                    $this->replaceListValues($search, $domNode, $valuesList[0]);
                }
    ...
    protected function variable2Text($variables, $dom, $options)
    {
        ...
        foreach ($variables as $variable => $value) {
            ...
            foreach ($foundNodes as $node) {
                ...
                $strNode = str_replace($search, htmlspecialchars($value), $strNode);
