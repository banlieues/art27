...
use Phpdocx\Elements\WordFragmentExtended;
...
class CreateDocxFromTemplate extends CreateDocx
{
        ...
        if ($target == 'document') {
            ...
            foreach ($foundNodes as $node) {
                ...
                if ($numIlvlNode->length > 0) {
                    $this->replaceListValues($search, $domNode, $listValues, (int)$numIlvlNode->item(0)->getAttribute('w:val'));
                } else {
                    $this->replaceListValues($search, $domNode, $listValues);
                }
            ...
            if (isset($options['parseLineBreaks']) && $options['parseLineBreaks']) {
                $this->_wordDocumentC = str_replace('__LINEBREAK__', '</w:t><w:br/><w:t xml:space="preserve">', $this->_wordDocumentC);
            }
            ...
            foreach ($xpathHeadersResults as $headersResults) {
                ...
                if (!empty($loadContent)) {
                    ...
                    $stringDoc = $dom->asXML();
                    if (isset($options['parseLineBreaks']) && $options['parseLineBreaks']) {
                        $stringDoc = str_replace('__LINEBREAK__', '</w:t><w:br/><w:t xml:space="preserve">', $stringDoc);
     ...
     * @param int $level
     */
    protected function replaceListValues($search, $domNode, $listValues, $level = 0) {
        foreach ($listValues as $key => $value) {
            if (is_array($value)) {
                $this->replaceListValues($search, $domNode, $value, $level + 1);
            } else {
    ...
    {
        if (!file_exists($src) && !strstr($src, 'base64,') && (!isset($options['streamMode']) || !$options['streamMode'])) {
        ...
        if (isset($options['replaceShapes']) && $options['replaceShapes']) {
            ...
            for ($i = 0; $i < $domImagesShapes->length; $i++) {
                if ($domImagesShapes->item($i)->getAttribute('alt') == self::$_templateSymbolStart . $variable . self::$_templateSymbolEnd && $imageShapeCounter == 0) {
                    //generate new relationship
                    $relString = '<Relationship Id="' . $ind . '" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/img' . $id . '.' . $extension . '" />';
                    if ($rels == 'document') {
                        if ($relsCounter == 0) {
    ...
    private function singleVariable4WordFragment($var, $val, $type, $loadContent, $firstMatch, $options = array())
    {
        ...
        if (file_exists(dirname(__FILE__) . '/../Elements/WordFragmentExtended.php')) {
            if (isset($options['stylesReplacementType'])) {
                if ($options['stylesReplacementType'] == 'usePlaceholderStyles' || $options['stylesReplacementType'] == 'mixPlaceholderStyles') {
                    ...
                    $val = $wordFragmentExtended->setStylesReplacementType($var, $val, $loadContent, $options);
                }
            }
        }
        ...
        foreach ($docNodes as $node) {
            ...
            if (strpos($cleanNodeText, $searchVariable) !== false) {
                if ($type == 'block') {
                    ...
                    $node->parentNode->insertBefore($cursorNode, $node);
                    $node->parentNode->removeChild($node);
                } else if ($type == 'inline-block' && file_exists(dirname(__FILE__) . '/../Elements/WordFragmentExtended.php')) {
                    $textNode = $node->ownerDocument->saveXML($node);
                    // get rPr tags of the placeholder to be applied to the next w:r of the content
                    $stylesRpr = '';
                    try {
                        $docDOMrPr = new \DOMDocument();
                        if (PHP_VERSION_ID < 80000) {
                            $optionEntityLoader = libxml_disable_entity_loader(true);
                        }
                        @$docDOMrPr->loadXML('<w:root xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas" 
                                               xmlns:mo="http://schemas.microsoft.com/office/mac/office/2008/main" 
                                               xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" 
                                               xmlns:mv="urn:schemas-microsoft-com:mac:vml" 
                                               xmlns:o="urn:schemas-microsoft-com:office:office" 
                                               ...
                                               xmlns:wne="http://schemas.microsoft.com/office/word/2006/wordml" 
                                               xmlns:wps="http://schemas.microsoft.com/office/word/2010/wordprocessingShape" 
                                               mc:Ignorable="w14 wp14">' . $textNode . '</w:root>');
                        if (PHP_VERSION_ID < 80000) {
                            libxml_disable_entity_loader($optionEntityLoader);
                        }
                    ...
                    if (self::$_templateSymbolStart == self::$_templateSymbolEnd) {
                        ...
                        $newNodeText = implode(self::$_templateSymbolStart, $textChunks);
                        $newNodeText = str_replace(self::$_templateSymbolStart . '</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">', '</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">', $newNodeText);
                        $newNodeText = str_replace('</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">' . self::$_templateSymbolStart, '</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">', $newNodeText);
                    } else {
                        $newNodeText = str_replace(self::$_templateSymbolStart . $var . self::$_templateSymbolEnd, self::$_templateSymbolStart . '</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">' . self::$_templateSymbolEnd, $textNode);
                        $newNodeText = str_replace(self::$_templateSymbolStart . '</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">', '</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">', $newNodeText);
                        $newNodeText = str_replace('</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">' . self::$_templateSymbolEnd, '</w:t></w:r><cursorWordML/><w:r>' . $stylesRpr . '<w:t xml:space="preserve">', $newNodeText);
                    }
                    $tempDoc = new \DOMDocument();
                    if (PHP_VERSION_ID < 80000) {
                        $optionEntityLoader = libxml_disable_entity_loader(true);
