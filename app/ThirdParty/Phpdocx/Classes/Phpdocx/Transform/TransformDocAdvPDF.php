...
class TransformDocAdvPDF
{
    ...
    {
        $this->htmlPlugin = new TransformDocAdvPDFPlugin();
        $this->css['body'] = '';
        $this->css['@page'] = '';
        ...
        // add blank space to empty paragraphs to avoid hidding them
        $this->html = str_replace(' "></p>', ' ">&nbsp;</p>', $this->html);

        $htmlContentToRender = $this->html;
        ...
        if ($endnotesContent) {
            ...
            $htmlContentToRender .= $endnotesContent;
            $this->html = '';
        }
        ...
        if ($footnotesContent) {
            ...
            $this->pdf->load_html($footnotesContent);
            $htmlContentToRender .= $footnotesContent;
            $this->html = '';
        }
        ...
        if ($commentsContent) {
            ...
            $this->pdf->load_html($commentsContent);
            $htmlContentToRender .= $commentsContent;
            $this->html = '';
        }

        $this->pdf->load_html($htmlContentToRender);
                    ...
                    if (isset($pprStyle->tagName)) {
                        switch ($pprStyle->tagName) {
                            ...
                            case 'w:pageBreakBefore':
                                if ($pprStyle->getAttribute('w:val') == 'on' || $pprStyle->getAttribute('w:val') == '1' || $pprStyle->getAttribute('w:val') == 'true' || !$pprStyle->hasAttribute('w:val')) {
            ...
            if ($node->parentNode && $node->parentNode->tagName == 'w:p') {
                ...
                if ($rprParentpPrStyles->length > 0) {
                    ...
                    if ($rprParentpPrrPrStyles->length > 0) {
                        ...
                        if ($rprParentpPrrPrBStyles->length > 0 && ($rprParentpPrrPrBStyles->item(0)->getAttribute('w:val') == 'on' || $rprParentpPrrPrBStyles->item(0)->getAttribute('w:val') == '1' || $rprParentpPrrPrBStyles->item(0)->getAttribute('w:val') == 'true' || !$rprParentpPrrPrBStyles->item(0)->hasAttribute('w:val'))) {
                ...
                foreach ($rprStyles->item(0)->childNodes as $rprStyle) {
                    if (isset($rprStyle->tagName)) {
                        switch ($rprStyle->tagName) {
                            case 'w:b':
                                if ($rprStyle->getAttribute('w:val') == 'on' || $rprStyle->getAttribute('w:val') == '1' || $rprStyle->getAttribute('w:val') == 'true' || !$rprStyle->hasAttribute('w:val')) {
                                    $styles .= 'font-weight: bold;';
                                } else if ($rprStyle->getAttribute('w:val') == 'off' || $rprStyle->getAttribute('w:val') == '0' || $rprStyle->getAttribute('w:val') == 'false') {
                                    $styles .= 'font-weight: normal;';
                                }
                                break;
                            case 'w:caps':
    ...
    protected function addSection()
    {
        ...
        $this->css['@page'] .= 'margin:' . $sectionMarginTop . ' ' . $sectionMarginRight . ' ' . $sectionMarginBottom . ' ' . $sectionMarginLeft . ';';
        ...
        if ($styles->length > 0) {
            foreach ($styles as $style) {
                ...
                switch ($style->getAttribute('w:type')) {
                    ...
                    case 'paragraph':
                        // basedOn tag
                        $basedOnStyle = $style->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'basedOn');
                        if ($basedOnStyle->length > 0) {
                            $basedOnStyleValue = $basedOnStyle->item(0)->getAttribute('w:val');
                            if (isset($this->css[$style->getAttribute('w:type') . '_' . $basedOnStyleValue])) {
                                $this->css[$nodeClass] .= $this->css[$style->getAttribute('w:type') . '_' . $basedOnStyleValue];
                            }
    ...
    protected function transformW_BR($childNode, $nodeClass)
    {
        if ($childNode->hasAttribute('w:type') && $childNode->getAttribute('w:type') == 'page') {
            $this->html .= '<p style="page-break-before: always;"></p>';
        } else {
    ...
    protected function transformW_COMMENTREFERENCE($childNode, $nodeClass)
    {
        ...
        if (!$childNode->hasAttribute('w:customMarkFollows')) {
            ...
            $this->html .= '<a href="#comment-' . $childNode->getAttribute('w:id') . '" style="vertical-align: super;">' . '[COMMENT ' . count($this->commentsIndex) . ']' . '</a> ';
        }
    }
    ...
    protected function transformW_ENDNOTEREFERENCE($childNode, $nodeClass)
    {
        ...
        if (!$childNode->hasAttribute('w:customMarkFollows')) {
            ...
            $this->html .= '<a href="#endnote-' . $childNode->getAttribute('w:id') . '" style="vertical-align: super;">' . strtolower($return) . '</a> ';
        }
    }
    ...
    protected function transformW_FOOTNOTEREFERENCE($childNode, $nodeClass)
    {
        ...
        if (!$childNode->hasAttribute('w:customMarkFollows')) {
            ...
            $this->html .= '<a href="#footnote-' . $childNode->getAttribute('w:id') . '" style="vertical-align: super;">' . count($this->footnotesIndex) . '</a> ';
        }
    }
    ...
    protected function transformW_HYPERLINK($childNode, $nodeClass)
    {
        ...
        $this->html .= '<' . $this->htmlPlugin->getTag('hyperlink') . ' class="'.$nodeClass.' ' . ($this->htmlPlugin->getExtraClass('hyperlink')==null?'':$this->htmlPlugin->getExtraClass('hyperlink')) . '" href="'.$href.'" target="_blank">';
    ...
    protected function transformW_P($childNode, $nodeClass)
    {
        ...
        if ($numPrTag->length > 0) {
            ...
            if ($numIdTag->length > 0 && $numIdTag->item(0)->hasAttribute('w:val') && $numIdTag->item(0)->getAttribute('w:val') != '') {
                ...
                if (!isset($this->numberingParagraph['value'][$numberingLevel . $numIdTag->item(0)->getAttribute('w:val')]) || ($this->numberingParagraph['level'] != $numberingLevel && (int)$this->numberingParagraph['level'][$numIdTag->item(0)->getAttribute('w:val')] < (int)$numberingLevel)) {
        ...
        if ($childNode->hasChildNodes()) {
            if ($elementTag == $this->htmlPlugin->getTag('itemList')) {
                ...
                if ($pPrTag->length > 0) {
                    // numbering styles
                    if (isset($numIdTag) && $numIdTag && $numIdTag->length > 0) {
                        $numberingLevelTags = $this->getNumberingStyles($numIdTag->item(0)->getAttribute('w:val'), $numberingLevel);
                        $this->css[$nodeClass] .= $this->addPprStyles($numberingLevelTags);
                    }
    ...
    protected function transformW_R($childNode, $nodeClass)
    {
        ...
        if ($this->complexField === null || (isset($this->complexField['type']) && $this->complexField['type'] != 'HYPERLINK')) {
