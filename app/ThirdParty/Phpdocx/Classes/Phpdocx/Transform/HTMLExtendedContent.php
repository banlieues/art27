...
class HTMLExtendedContent
{
     ...
     * Constructor
     */
    public function __construct(\Phpdocx\Create\CreateDocx $docx)
    {
        $this->docx = $docx;
    }
    ...
    public function getContent($node, $method, $openP, $attributesContent = array())
    {
        if (isset($attributesContent['target'])) {
            $wordFragment = new \Phpdocx\Elements\WordFragment($this->docx, $attributesContent['target']);
        } else {
            $wordFragment = new \Phpdocx\Elements\WordFragment($this->docx);
        }
