...
class TransformDocAdvHTML
{
    ...
    public function transform(TransformDocAdvHTMLPlugin $htmlPlugin, $options = array())
    {
        ...
        foreach ($headersContents as $headerContent) {
            if (!empty($headerContent['content'])) {
        ...
        foreach ($footersContents as $footerContent) {
            if (!empty($footerContent['content'])) {
        ...
        if (is_array($this->numberingParagraph)) {
            ...
            if ($numPrTag->length > 0) {
                ...
                if ($numIdTag->length > 0 && $numIdTag->item(0)->hasAttribute('w:val') && $numIdTag->item(0)->getAttribute('w:val') != '') {
                    ...
                    if (!isset($this->numberingParagraph['value'][$numberingLevel . $numIdTag->item(0)->getAttribute('w:val')]) || ($this->numberingParagraph['level'][$numIdTag->item(0)->getAttribute('w:val')] != $numberingLevel && (int)$this->numberingParagraph['level'][$numIdTag->item(0)->getAttribute('w:val')] < (int)$numberingLevel)) {
            ...
            if ($numPrTag->length > 0) {
                ...
                if ($numIdTag->length > 0 && $numIdTag->item(0)->hasAttribute('w:val') && $numIdTag->item(0)->getAttribute('w:val') != '') {
                    ...
                    if ($previousSiblingElement !== null && !($previousSiblingElement instanceof \DOMText)) {
    ...
    protected function transformW_R($childNode, $nodeClass)
    {
        ...
        if ($childNode->nodeValue === '' && $tTags->length < 1 && (isset($this->complexField['type']) && $this->complexField['type'] != 'HYPERLINK')) {
            $this->html .= '&nbsp;';
        }
    ...
    protected function transformW_SECTPR($childNode, $nodeClass)
    {
        ...
        if (!empty($headerContentSection)) {
            // remove the header placeholder to add the header contents to the correct place
    ...
    protected function transformW_SDT($childNode, $nodeClass)
    {
        ...
        switch ($lastChildSdtPr->tagName) {
            ...
            default:
                ...
                // handle child elements
                if ($sdtContentNode->hasChildNodes()) {
                    $this->transformXml($sdtContentNode);
                } else {
