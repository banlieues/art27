...
if (!isset($phpdocxconfig['transform']['path']) || !is_readable(str_replace('\'', '', $phpdocxconfig['transform']['path'])) || !is_file(str_replace('\'', '', $phpdocxconfig['transform']['path']))) {
    if ($isWeb) {
         $output .= '<span class="testko">';
    }
    $output .= 'Error ';
...
$output .= 'This native conversion method includes a set of supported contents and styles detailed on:' . $break;
if ($isWeb) {
    $output .= '</p>';
}
...
$output .= 'phpdocx requires DOMPDF to transform DOCX to PDF using the native conversion plugin:' . $break;

if ($isWeb) {
    $output .= '
    <textarea onClick="this.select();">
require_once \'Classes/Phpdocx/Create/CreateDocx.php\';'.'
// download and load Dompdf (https://github.com/dompdf/dompdf) using Composer or the autoloader included in the package of the library: 
