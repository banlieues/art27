    ...
    <div class="col-12">
        <div class="card border-top-amethyst">
            ...
            <?php if ($nbDemandes>0): ?>
              ...
              <p class="m-5 text-center"> <i>Choississez une demande et cliquer sur enregister en haut de la page pour l'associer au partenaire_culturel</i></p>

                <div class="table-responsive"> 
                <form id="new_form" method="post" action="<?=base_url()?>/partenaire_culturel/save_associe_demande" >
                    <input type="hidden" name="id_partenaire_culturel" value="<?=$partenaire_culturel->id_partenaire_culturel?>">
                    <table id="table_data" class="table table-striped table-hover my-0 table-sm">
                        ...
                        <?php foreach($demandes as $demande):?>
                            ...
                            >
                                 <td><input name="id_personne_partenaire_culturel" value="<?=$demande->id_personne_partenaire_culturel?>" type="radio"></td>
                                      ...
                                      <?php foreach($contacts as $contact):?>
                                        <?php if(!empty($contact->nom_contact)||!empty($contact->prenom_contact)):?>
                                                ...
                                                <?php echo $themes->contact->icon;?> <?=strtoupper($contact->nom_contact)?> <?=$contact->prenom_contact?>
                                                    <?php if(!empty($contact->relation_partenaire_culturel)):?>
                                                        (<?=$contact->relation_partenaire_culturel;?>)
                                                     <?php endif;?>   
                                                </a>
                                        <?Php endif;?>
                                ...
                                <td>
                                    <?php
                                       $partenaire_culturels_culturel= $demandeModel->getPartenaire_culturels_culturel($demande->id_demande);
                                       //debug($partenaire_culturels_culturel);
                                    ?>
                                    <?php if(!empty($partenaire_culturels_culturel)):?>
                                        <?php foreach($partenaire_culturels_culturel as $partenaire_culturel):?>
                                            <?php if(!empty($partenaire_culturel->adresse_fr)):?>
                                                <a href="<?=base_url()?>/partenaire_culturel/fiche/<?=$partenaire_culturel->id_partenaire_culturel?>" class="btn btn-secondary btn-sm mb-2">
                                                <?php echo $themes->partenaire_culturel->icon;?>
                                                    <?=$partenaire_culturel->adresse_fr?>
                                                </a>
                                            <?php endif;?>    
                                        <?php endforeach;?>    
