...
//			endif;
		...
		public function get_messages($params=array())
		{
			...
			if(isset($params['nolus']) && $params['nolus']==TRUE)
            {
                ...
                $builder->join("email_outlook_lien","email_outlook.id_primary = email_outlook_lien.id_email","left");
                $builder->join("demande","demande.id_demande=email_outlook_lien.id_demande","left");
               ...
               /* EXISTS (
                    ...
                    FROM email_outlook_lien
                    WHERE email_outlook.id_primary = email_outlook_lien.id_email 
                    email_outlook_lien.id_demande IN  ('.$id_requete_demandes.')
                )*/
...
//                                    print_r($id_hge_inscr); print_r($inscr); print_r($this->db->last_query()); die();
            ...
            public function update_calendar_outlook($id_rdv){
                ...
                $left_condition['contact']='contact.id_contact=contact_rdv.id_contact';
                $left_condition['contact_profil']='contact_profil.id_contact_profil=contact_rdv.id_contact_profil';
                 array_push($left, $left_condition);

                $personnes=$this->ban_crud_model->read_data('contact_rdv',$fields, 'contact_rdv.id_rdv='.$id_rdv, $left);
               // debugd($rdvA[0]);
