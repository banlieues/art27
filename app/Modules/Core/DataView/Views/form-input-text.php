...
<?php if(!in_array($index,$burgis_champs)):?>
        <?php if(in_array($index,$no_direct_modif_indexes)&&!empty($value)):?>
            <input disabled  name="<?=$index?>" type="text" class="form-control" value="<?= $value?>">
        <?php else:?>
        <input  name="<?=$index?>" type="text" class="form-control" value="<?= $value?>">
        <?php endif;?>
<?php else:?>
    ...
    <?php if($index=="adresse_nl_bien"||$index=="adresse_nl_bien_no_modif" ): $lg="NL"; else: $lg="FR"; endif; ?>
                ...
                <input autocomplete="off" name="<?=$index?>" type="text" id="input_dao" class="search_address form-control input_dao" value="<?= $value?>" placeholder="Adresse <?=$lg?>" table="bien" key="id_bien" field_sql="adresse_fr">
