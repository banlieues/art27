<?php $request = \Config\Services::request(); ?>
        ...
        <?php foreach($indexes as $index):?>
            ...
            <?php else:?>
                ...
                <div <?php if($typeDataView!="modelisation"):?>class="tr_fiche_<?=$index?> tr_fiche" index="<?=$index?>" <?php endif;?>>
                    <div class="row mb-2 container_one_field <?php if($typeDataView!="modelisation"):?>formcourscopy<?php endif;?>">
                        ...
                        <?php endif;?>
                                <?php if($typeDataView=="modelisation"):?>
                                    <div class="row">
                                        <?php if(isset($fields[$index])):?>
                                            <div class="col"><i index="<?=$index?>" style="cursor:pointer" class="<?=icon("minus-field")?> text-danger deleteField"></i></div>
                                            <div class="col-10"><b><?php if($dataView->isRequired($fields[$index])):?>*<?php endif;?><?=$fields[$index]["label"];?></b></div>
                                        <?php endif;?>
                                    </div>
                                <?php else:?>    
                                    <?php if(isset($fields[$index])):?>
                                        <label style='font-weight:bold' class='control-label'><?php if($dataView->isRequired($fields[$index])):?>*<?php endif;?><?=$fields[$index]["label"];?></label>
                                    <?php endif;?>
                                <?php endif;?>        
                            </div>
                            ...
                            <?php else://Form ?>
                                ...
                                <?php if($validation->getError($index)&&$typeDataView!="read"): //Error message for form only ?>
                                    ...
                                    <span class="text-danger"><i class="<?=icon("triangle_warning")?>"></i> <?= $error = $validation->getError($index); ?></span>
